...
### Added
- **Research Campaign HTTP API (M4 wiring)** — `§7.2` campaign routes mounted in `api_server.py` (lazy controller, `api_server` stays a thin assembler): create/list/get/pause/resume/cancel campaigns, plus experiments/candidates/repairs/reports/latest sub-resources.
- **research_controller.contracts (M0)** — DSA `research-loop.v1` contract bundle copy under `agent/src/research_controller/contracts/`:
  - JSON Schema + golden fixture copies generated from the DSA `contract_bundle.v1`
  - Pydantic client models generated by `datamodel-code-generator` (never hand-edited)
  - `source_bundle_sha256` drift detection + `regenerate.py` to rebuild from a fresh bundle
